#!/usr/bin/env python3
"""Mesh Zoo Lab: a keyless multi-family panel from Kaggle-hosted weights.

Four model families attached as local weights (no API keys anywhere):
  google/gemma-2-2b-it  — the reference instrument
  google/gemma-3-1b-it  — small fresh Gemma writer
  metaresearch/llama-3.2-3b-instruct — the strongest keyless writer here
  qwen-lm/qwen2.5-1.5b-instruct — fourth family

Experiments:
  1. FRAME DUEL v2 — every zoo model writes frames; the Gemma instrument scores
     each frame by null-controlled surprisal collapse. Explanation deficit per
     writer vs ground truth.
  2. CROSS-INSTRUMENT INVARIANCE — re-measure S/R with Llama as the instrument;
     if the laugh region is instrument-invariant, the theory isn't an artifact
     of one model's logits.
  3. CENTURY TEST — 1916 jest-book items (Project Gutenberg, public domain)
     measured today: do universal-mechanics frames survive while dead-cache
     frames die? (THEORY.md §11.)
Outputs: /kaggle/working/research_out/zoo_report.json
"""
...
CELLS = [
    md(
        ...
        "Four model families as **local Kaggle weights** — no API keys: Gemma-2-2B (reference "
        "instrument), Gemma-3-1B, Llama-3.2-3B, Qwen2.5-1.5B.\n\n"
        "1. **Frame duel v2**: every family writes frames; the instrument scores them in nats "
        "(null-controlled).\n"
        "2. **Cross-instrument invariance**: S/R re-measured off Llama's logits — is the laugh "
        "region a property of jokes, or of one model?\n"
    ...
    md("## 1. Frame duel v2 — scored by the Gemma-2 reference instrument"),
    code(
        "load('gemma-2-2b')\n"
        "duel = {}\n"
    ...
    md("## 2. Cross-instrument invariance — Llama measures what Gemma measured"),
    code(
    ...
    md("## 3. Century test — 1916 jest-book items measured today (public domain)"),
    code(
        "century = []\n"
        "try:\n"
    ...
    md(
        ...
        "- **Invariance**: if S/R orderings agree across instruments, the laugh region is a property "
        "of the jokes, not an artifact of Gemma's logits — the theory's strongest robustness card.\n"
        "- **Century test**: frames renting universal mechanics survive 110 years; frames renting "
        "dead caches don't (THEORY.md §11's first falsifiable temporal prediction)."
    ),
]
...
def main() -> None:
    ...
    (HERE / "zoo_lab.ipynb").write_text(json.dumps(nb, indent=1), encoding="utf-8")
    print("wrote", HERE / "zoo_lab.ipynb", f"({len(CELLS)} cells)")
