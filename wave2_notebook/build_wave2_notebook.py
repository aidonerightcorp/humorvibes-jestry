...
The hackathon-era writeup could not be submitted separately, so the notebook
BECAME the writeup: theory, method, data provenance, results and limits are
markdown cells here, interleaved with the code that produces the numbers they
quote. The deadline has since passed and no competition submission is claimed;
the notebook is maintained as the canonical executable write-up.
...
CELLS: list[tuple[str, str]] = [
    ...
    ("markdown",
     ...
     "| Question | Answer |\n"
     "|---|---|\n"
     "| **What problem are we solving?** | Humor research and humor apps often collapse three "
     "different things—text structure, model expectation, and human response—into one unsupported "
     "‘funniness score.’ Source imbalance, English-shaped filters, contextual labels, and tiny "
     "samples can then produce confident but irreproducible conclusions. |\n"
     "| **What is the proposed solution or exploration?** | Build a provenance-rich multilingual "
     "corpus; keep form, topic, and source-declared style separate; measure one narrow quantity "
     "directly from Gemma logits; compare controlled arms with uncertainty; and evaluate human "
     "labels with contest-held-out splits. The motivating surprise-reduction account is treated "
     "as a falsifiable framework, not a completed brain result. |\n"
     "| **What did we learn?** | The Gemma instrument reproduces its pinned check, but the joke-form "
     "study does **not** establish separation from the proverb control. Text-only caption features "
     "recover a small part of a context-limited human signal. Several of the strongest findings "
     "are data and evaluation failures that were caught and corrected. |\n"
     "| **What can people use this for?** | A rights-aware research corpus, reproducible measurement "
     "and grouped-evaluation patterns, multilingual taxonomy fixtures, machine-readable receipts, "
     "and an SDK/API for building retrieval, generation, and analysis applications. |\n\n"
     "### Controlling outputs\n\n"
     "| Output | Result | Meaning |\n"
     "|---|---:|---|\n"
     "| Full inventory / public slice | **3,164,600 / 121,670 rows** | Broad local evidence inventory; "
     "only explicitly redistributable text is republished. |\n"
     "| Instrument check | **S = 3.188 over 10 tokens** vs pinned 3.19 | The attached Gemma path agrees "
     "with the reference closely enough to continue. |\n"
     "| Form study | **0/10** joke-form intervals strictly above control; **10/10 overlap** | "
     "**SEPARATION IS NOT ESTABLISHED**. The mean ordering is only a hypothesis. |\n"
     "| Caption model | **rho = 0.1555**, **37.8%** of measured text-only bound | Structural text "
     "features transfer weakly; caption response depends heavily on drawing and contest context. |\n\n"
     "### What this notebook does not claim\n\n"
     "It is **not** a universal funniness detector, joke leaderboard, content-safety classifier, "
     "or proof that a form causes laughter. `S` is Gemma surprisal. Source-specific votes are noisy "
     "human signals, not interchangeable grades. A configured app model is a capability, not a "
     "validated scientific instrument.\n\n"
    ...
    ("markdown",
     ...
     "This project does not assume that humor can be reduced to one score. It tests a narrower, "
     "falsifiable structural hypothesis and keeps each evidence type separate.\n\n"
     "**Status.** The Build with Gemma deadline has passed and no competition submission is "
     "claimed; this notebook is maintained as the canonical executable write-up of an open "
     "research project.\n\n"
     "**Starting point.** Predictive-coding and predictive-processing accounts model perception "
     "in terms of hierarchical expectations and errors. Friston's free-energy principle is a "
     "broader framework in which adaptive agents minimize variational free energy. The memorable "
     "phrase ‘the brain is a surprise-reduction engine’ is useful shorthand, but it is not a "
     "literal result this notebook establishes. Variational free energy, Shannon surprisal, token "
     "negative log-likelihood, a person's felt surprise, and funniness are not interchangeable.\n\n"
     "**Project hypothesis.** A joke can be a controlled prediction error with a compact, "
     "audience-permitted repair.\n\n"
     ...
     + B_DEFINITION + "\n\n"
     "Each proposed failure mode becomes diagnosable: predictable (low S), nonsense (high S, no "
     "frame), dissected frog (frame too expensive), offence (B collision). This is a research "
     "program, not a completed causal or neural theory: Wave 2 directly measures model-specific "
     "S, organizes evidence for R/E/B, and reports where the available data cannot decide the "
     "broader claim.\n\n"
     "**Grounding sources.** Information-theoretic language expectation: "
     "[Hale (2001)](https://aclanthology.org/N01-1021/) and "
     "[Smith & Levy (2013)](https://doi.org/10.1016/j.cognition.2013.02.013). Predictive coding: "
     "[Rao & Ballard (1999)](https://doi.org/10.1038/4580). Free-energy principle and its scope: "
     "[Friston (2010)](https://doi.org/10.1038/nrn2787). Incongruity and resolution: "
     "[Shultz (1972)](https://doi.org/10.1016/0022-0965(72)90074-4). Benign violation: "
     "[McGraw & Warren (2010)](https://doi.org/10.1177/0956797610376073). The repository's "
     "research-foundation document maps what each source supports—and what it does not."),

    ("markdown",
     "## Gemma as instrument, not oracle\n\n"
     "A causal language model exposes its own next-token distribution, so this project never asks "
     ...
     "That distinction is the whole method. A rating is an output the model was trained "
     "to produce; a log-probability is a measurement of the frozen model's own expectation. "
     "It can falsify a model-level prediction, not a human or neural claim.\n\n"
    ...
    ("markdown",
     "## From starting framework to useful claim\n\n"
     "The diagram makes the evidence boundary explicit. Prior theory motivates the comic "
     "hypothesis; model instruments test model-level quantities; a preregistered human study is "
     "required before any writer or audience benefit claim. No model score crosses that gate.\n\n"
     "The repository now includes a privacy-minimized within-writer crossover protocol and local "
     "clustered analyzer. Its synthetic demo intentionally contains an apparent positive effect "
     "but can never become claim-ready."),

    ("code",
     "from IPython.display import SVG, display\n"
     "display(SVG(filename='docs/figures/surprise-to-product.svg'))\n"
     "from humorvibes.studies import synthetic_demo_receipt\n"
     "study_demo = synthetic_demo_receipt()\n"
     "print('study demo origin:', study_demo['data_origin'])\n"
     "print('apparent synthetic effect:', round(study_demo['estimate'], 3))\n"
     "print('claim ready:', study_demo['claim_gate']['claim_ready'])\n"
     "print('allowed claim:', study_demo['allowed_claim'])\n"
     "assert study_demo['claim_gate']['claim_ready'] is False"),
    ...
    ("markdown",
     ...
     "**Question:** What evidence exists, what may be redistributed, and how concentrated is it? "
     "**Output:** a verified full-corpus census plus a deterministic, rights-filtered public "
     "slice. **Learning:** source-family concentration and licence boundaries must be measured "
     "before any corpus-wide average is interpreted.\n\n"
    ...
    ("markdown",
     "## 2. Styles of joke\n\n"
     "**Question:** Can structure, topic, and source-declared tradition be studied without "
     "treating them as the same label? **Output:** three independent axes—form, domain, and "
     "style category. **Learning:** source-declared style is the strongest of the three; inferred "
     "form coverage remains sparse and English-biased even after multilingual expansion.\n\n"
    ...
    ("markdown",
     ...
     "**Question:** Can the expected frame and its violation be represented separately instead "
     "of inferred from a single joke string? **Output:** independently annotated frame pairs and "
     "free-text repair explanations. **Learning:** these artifacts make frame-repair hypotheses "
     "testable, but they are evidence inputs—not human funniness labels.\n\n"
     ...
     "a stated violation, annotated independently, before any caption exists. Across those "
     "705 cartoons the layer yields **2,581 expectation/violation annotation rows** (≈3.7 "
     "rows per cartoon) — '705' counts cartoons, '2,581' counts annotation rows; both "
     "numbers describe this one artifact. A further 647 "
    ...
    ("markdown",
     ...
     "**Question:** Is a punchline less predictable to a fixed model under its setup, and does "
     "that quantity differ reliably by form? **Output:** a pinned instrument check, token-level "
     "surprisal, and interval-based form comparisons. **Learning:** the instrument reproduces; "
     "the claimed form separation does not.\n\n"
    ...
    ("markdown",
     ...
     "**Question:** How much source-specific human response can structural text features recover "
     "on unseen contest contexts? **Output:** a label ceiling, a text-only bound, and a "
     "contest-held-out model score. **Learning:** the signal is real but weakly portable; most of "
     "what makes a caption land is not recoverable from its words alone.\n\n"
     ...
     "the text-only bound; 358 of 360 contests score above chance. The old "
    ...
    ("markdown",
     "## 6. What we learned\n\n"
     "The most useful outcome is not a winning joke form. It is a clearer map of what the "
     "available evidence can and cannot support.\n\n"
     "| Learning | Evidence | Status |\n"
     "|---|---|---|\n"
     "| A fixed Gemma path can serve as a reproducible expectation instrument. | The attached "
     "model returns **S = 3.188 over 10 tokens** against the pinned 3.19 reference. | Supported "
     "for this checkpoint and text; not a universal human measure. |\n"
     "| The observed form ordering is not established as a separation. | **0/10** joke-form "
     "intervals lie strictly above the proverb control and **10/10 overlap** it. | Null result; "
     "more data would test the ordering as a new hypothesis. |\n"
     "| Caption response is strongly contextual. | The held-out model reaches **0.1555**, or "
     "**37.8%** of the measured text-only bound. | Structural text features carry some signal, "
     "but do not recover most reception. |\n"
     "| Data assumptions changed results more than model tuning did. | Script-aware length "
     "screening restored CJK records; family grouping exposed concentration; excluding the "
     "length-only `shaggy_dog` proxy from specific-form analysis removed a source-verbosity "
     "confound (the label itself is still emitted). | Strong operational "
     "learning with regression coverage. |\n"
     "| One style label is not enough. | Form, lexical domain, and source-declared style answer "
     "different questions and have different error modes. | Reusable taxonomy design; coverage "
     "and language limits remain explicit. |\n"
     "| Uncertainty and provenance are part of the result. | Bare means suggested a clean story; "
     "intervals and source inspection removed it. | Methodological conclusion supported by the "
     "corrected receipts. |"),

    ("code",
     "# A compact, machine-readable handoff of the notebook's controlling outputs.\n"
     "executive_summary = {\n"
     "    'schema_version': 1,\n"
     "    'project': 'Humor Genome Wave 2',\n"
     "    'problem': (\n"
     "        'Humor datasets and applications often conflate text structure, model expectation, '\n"
     "        'and noisy contextual human response into an unsupported funniness score.'\n"
     "    ),\n"
     "    'exploration': [\n"
     "        'provenance-rich multilingual corpus with a rights-filtered public slice',\n"
     "        'separate form, domain, and source-declared style axes',\n"
     "        'teacher-forced Gemma surprisal with a pinned instrument check',\n"
     "        'interval-based form controls and contest-held-out human-label evaluation',\n"
     "    ],\n"
     "    'outputs': {\n"
     "        'release_verification': release_receipt['status'],\n"
     "        'full_inventory_rows': c['items'],\n"
     "        'published_rows': sample_c['items'],\n"
     "        'aligned_phrase_pairs': len(aligned),\n"
     "        'expectation_violation_frames': len(frames),\n"
     "        'instrument': {\n"
     "            'metric': 'mean punchline token NLL given setup',\n"
     "            'S': round(r['S'], 3),\n"
     "            'tokens': r['n_tokens'],\n"
     "            'pinned_S': PINNED_S,\n"
     "            'agreement': ok,\n"
     ...
     "        },\n"
     "        'form_study': {\n"
     "            'measurements': sum(e['n'] for e in full['per_form'].values()),\n"
     "            'joke_forms': len(full_jokes),\n"
     "            'strictly_above_control': len(strict),\n"
     "            'overlap_control': len(overlap),\n"
     "            'verdict': 'SEPARATION IS NOT ESTABLISHED',\n"
     "        },\n"
     "        'caption_model': {\n"
     "            'captions': caption_model_receipt['n_rows'],\n"
     "            'held_out_contests': caption_model_receipt['n_contests'],\n"
     "            'spearman': model_result,\n"
     "            'text_only_bound': text_bound,\n"
     "            'fraction_of_text_only_bound': bounds['achieved_over_text_only_bound'],\n"
     "        },\n"
     "    },\n"
     "    'learnings': [\n"
     "        'the pinned Gemma expectation instrument reproduces',\n"
     "        'form separation from the proverb control is not established',\n"
     "        'caption response is only weakly recoverable from structural text features',\n"
     "        'source, licence, script, and grouping assumptions materially change conclusions',\n"
     "    ],\n"
     "    'uses': [\n"
     "        'help comedians and writers retrieve precedents and create testable variations',\n"
     "        'reproduce or challenge the measurements',\n"
     "        'build provenance-aware multilingual humor datasets',\n"
     "        'benchmark forms, embeddings, retrieval, or grouped evaluation',\n"
     "        'integrate generation, embeddings, similarity, and signals through the SDK/API',\n"
     "        'run a privacy-minimized paired writer-study protocol and evidence-gated analyzer',\n"
     "    ],\n"
     "    'not_for': [\n"
     "        'universal funniness scoring',\n"
     "        'ranking people, cultures, or joke forms',\n"
     "        'content-safety certification',\n"
     "        'claiming model output is human laughter',\n"
     "        'treating a synthetic study fixture as evidence of product advantage',\n"
     "    ],\n"
     "    'next_evidence_gate': (\n"
     "        'preregistered within-writer crossover trial with blinded opt-in audience evaluation'\n"
     "    ),\n"
     "}\n"
     "assert executive_summary['outputs']['instrument']['agreement']\n"
     "assert executive_summary['outputs']['form_study']['strictly_above_control'] == 0\n"
     "assert executive_summary['outputs']['form_study']['overlap_control'] == 10\n"
     "rendered_summary = json.dumps(executive_summary, indent=2, ensure_ascii=False, sort_keys=True)\n"
     "print(rendered_summary)\n"
     "working = Path('/kaggle/working')\n"
     "if working.is_dir():\n"
     "    summary_path = working / 'humor_genome_wave2_executive_summary.json'\n"
    ...
    ("markdown",
     ...
     "styles separate from the same proverb-control recipe (either direction) — but the design "
     "is receipted as UNDERPOWERED at n=12/group (the separation criterion could not have "
     "fired); a separate any-difference test among the styles gives p = 0.45, and token length "
     "is an uncontrolled covariate. Zero instrument errors. |\n"
     "| Caption divisiveness | `divisiveness_study.json` | The vote-histogram *shape* is a real "
     "label overall (split-half reliability ~0.51 vs ~0.67 for the mean) but text predicts it "
     "at ~17% of its ceiling vs ~19% for the mean — measurable, not easier. Vote bins are "
     "outcome strata; low-vote bins are not estimable. |\n"
     "| Demographic word norms | `demographic_norms_study.json` | Two crowds agree on word "
     "funniness (ρ = 0.414) yet single-word demographic gaps are NOT DETECTABLE at these "
     "per-word n (2/4,997 sex under Welch-t+FDR, 0/4,997 age; per-word reliability ≈ 0) — "
     "persona conditioning keeps only weak lexical support. |\n"
    ...
    ("markdown",
     ...
     "Retrieve precedents and generate or compare bounded candidate variations. | A model score "
     "cannot select what will work on stage; use blinded rehearsal or live response. |\n"
     "| Audience member | Explanations, form discovery, and explicit user-controlled preferences "
     "in a consent-based app | Explore why material can resolve differently for different people. "
     "| Do not infer taste, identity, values, or sensitive traits from demographic proxies. |\n"
     "| Academic | This notebook plus versioned receipts | Reproduce the null result, test a "
     "new control, or compare another fixed model instrument. | Do not compare uncalibrated S "
     "values across tokenizers or call S funniness. |\n"
     "| Data curator or archivist | Public corpus, census, manifest, aligned phrases, and frame "
     "pairs | Build "
     "rights-aware multilingual fixtures and audit source concentration. | Honour each row's "
     "licence and preserve source-family identity. |\n"
     "| Product or venue team | `humorvibes` Python SDK, FastAPI, OpenAPI, Docker, Compose, "
     "Kustomize, and Helm surfaces in the repository | Add allowlisted generation, embeddings, "
     "similarity, retrieval, or research signals to an app. | Application outputs are not the "
     "published Gemma measurement or human validation. |\n"
     "| Educator or student | The correction trail and adversarial tests | Teach why grouped "
     "splits, uncertainty, provenance, and negative results matter. | Treat corrected mistakes as "
     "part of the evidence, not as hidden implementation history. |\n\n"
     "These are the same six personas defined in `docs/PRODUCT_AND_RESEARCH_USE_CASES.md`. "
     "Contributors have their own lane — roadmap, expansion guide, notebook standard, and issue "
     "templates; a new capability is not a new scientific finding until it is executed and "
     "receipted.\n\n"
     "### Output map\n\n"
     "- **Readable result:** this executed notebook, with every controlling result next to its "
     "interpretation and limit.\n"
     "- **Downloadable result:** `humor_genome_wave2_executive_summary.json` in Kaggle output.\n"
     "- **Machine evidence:** versioned JSON/JSONL receipts under `jestry_out/`.\n"
     "- **Reusable data:** the public Kaggle dataset and its byte manifest.\n"
     "- **App contract:** the repository's generated OpenAPI document and typed Python client.\n\n"
     "The repository's `docs/PRODUCT_AND_RESEARCH_USE_CASES.md` maps these personas onto six "
     "proposed workflows with missing-evidence lists, success measures, and claim gates. The highest-value next "
     "experiment is a preregistered within-writer crossover trial with blinded, opt-in audience "
     "evaluation. `docs/REAL_WORLD_STUDY_WORKBENCH.md` makes that design executable: it aggregates "
     "ratings to material, compares paired conditions within writer and premise, bootstraps writers, "
     "rejects raw identity/material fields, and refuses claims from synthetic data.\n\n"
     "The deployment layer is intentionally downstream of the research layer. It makes the "
     "tools usable in an application without changing what this notebook measured."),
