...
CELLS: list[tuple[str, str]] = [
    ...
    (
        ...
        "\n"
        "def hard_tfidf_vectors(document_texts, query_texts):\n"
        "    import math\n"
        "    tokenized = [hard_tokens(text) for text in document_texts + query_texts]\n"
        ...
        "    return vectors[:len(document_texts)], vectors[len(document_texts):]\n"
        "\n"
        ...
        "    document_vectors, query_vectors = hard_tfidf_vectors(d.text.tolist(), q.text.tolist())\n"
        "    ranks, beats_frame, beats_context = [], [], []\n"
        "    document_ids = d.document_id.tolist()\n"
        "    for query_vector, (_, query) in zip(query_vectors, q.iterrows()):\n"
        "        scores = [sum(left * right for left, right in zip(query_vector, document_vector))\n"
        "                  for document_vector in document_vectors]\n"
        "        order = sorted(range(len(document_ids)), key=lambda index: (-scores[index], document_ids[index]))\n"
