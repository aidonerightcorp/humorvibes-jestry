...
| T2 | Raw S should not rank funniness | form study (Wave 2 notebook v14); ablation court | **Null, as the tenet itself predicts.** 0/10 joke-form CIs sit above the proverb control (all overlap); combined S/R/E/B vs human preference ρ = 0.033, CI [−0.126, 0.207] |
| T2 | Declared style occupies distinct S regimes | `declared_style_study.json` | **Null — replicating the form-study null on an independent label axis.** Seven community-declared styles (subreddit self-labels, 12 items each, zero annotation cost) against the same proverb-control recipe on the certified instrument: 0/7 CIs separate from control in either direction, any-difference permutation p = 0.45, zero instrument errors, calibration re-verified in-run (S = 3.1899/10) |
...
| T5 | Funniness varies by audience at the lexical level | `demographic_norms_study.json` | **Mostly null at word level.** Two crowds agree on which words are funny (ρ = 0.414) but single-word demographic gaps rarely survive FDR — Benjamini–Hochberg false-discovery-rate control — (9/4,997 sex; 0/4,997 age) and cross-dataset gap CIs include zero; one dimension-level survivor (sexual-connotation words skew younger). Persona-conditioning keeps only weak lexical support; joke-level human data still required |
| T6 | Text-only prediction has a hard ceiling | `caption_ceiling.json`, `caption_portability.json`, `caption_model.json` | **Confirmed (within-corpus).** Label ceiling 0.8262; only ~24% of a caption's standing travels with its words → text-only bound 0.4110; current text model 0.1555 median within contest (358/360 contests above chance). The gap worth chasing is the drawing |
| T6 | Divisiveness (the shape of disagreement) behaves as a target | `divisiveness_study.json` | **Measured; no free lunch.** Pole-conflict is a real label (split-half Spearman–Brown 0.51 overall at ≥40 votes, 0.53 at 160+ votes, vs 0.67/0.62 for the mean) but contest-held-out text features predict it no better than the mean: ~17% of its own ceiling vs ~19% for the mean. 12,605 integrity-inconsistent rows dropped, never averaged over |
...
| (craft) | Concrete word choice matters | `word_type_study.json` | **Supported (within-corpus).** Body-part words +0.205 over the pooled mean (p = 0.0002), food +0.125, animal +0.046; word-type features lift held-out Spearman 0.114 → 0.230. Not transfer-tested |
...
**9/4,997** and **0/4,997** significant gaps; declared-style **0/7 separate, permutation p = 0.45**;
divisiveness reliability **0.51** vs mean **0.67** at matched votes, both text-predicted at
**~17–19% of ceiling**. Sources: the receipt named beside each row above.
